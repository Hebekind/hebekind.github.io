...
require_once(getcwd().'/assets/vaneayoung/core.php');
require_once(getcwd().'/assets/vaneayoung/classes/class.openGraph.php');
require_once(getcwd().'/assets/vaneayoung/classes/class.S3.php');
require_once(getcwd().'/assets/vaneayoung/colors.php');
...
$this->cron_add_unmute = "{$this->settings['HOST']}/messenger.php?cmd=cron\&action=unmute\&recipient={$this->recipient}\&id={$this->userid}\&group={$this->group_id}\&page={$this->page_id}";
$this->cron_remove_unmute = "{$this->settings['HOST']}/messenger.php?cmd=cron\&action=unmute\&recipient={$this->recipient}\&id={$this->id}\&group={$this->group_id}\&page={$this->page_id}";
...
$content = $this->template->fetch($this->theme_dir."/messenger/group-markup.html");
...
$content = $this->template->fetch($this->theme_dir."/messenger/global-popup.html");
...
public function updatePeerId(){
	...
	$sql = $this->query_insert("INSERT INTO ".tbl_peer_id." (`userid`, `unique_id`, `useragent`) values ('{$uid}','{$peer_uniq_id}','{$useragent}') ON DUPLICATE KEY UPDATE `unique_id` = '{$peer_uniq_id}',`useragent`='{$useragent}'");
...
// set color
public function setColor($uid = 0){
	
	$color = isset($_POST['color']) ? $this->test_input($_POST['color']) : $this->settings['MESSENGER_DEFAULT_COLOR'];	
	$i = $this->USER['id'];
	$r = 0;
...
if (isset($_COOKIE['chattab_minimized'])) {
 ...
 if(!$this->checking($uid,$page_id,$group_id)){
	...
	$this->template->assign(['wo' => $wo, 'this' => $this, 'svgi' => $svgI, 'muted_arr' => ['contacts' => $muted_contacts, 'groups' => $muted_groups, 'pages' => $muted_pages], 'contacts' => $contacts, 'page_id' => $page_id, 'userid'=> $uid, 'group_id' => $group_id ]);
...
if($to_id === $this->userid) die("You can not send messages to yourself");
...
// check if the message exist
$q_check = $this->db->query("select `id`,`deleteby` from ".tbl_msg." where ".$sql_where." limit 1 ");
$r_check = $q_check->fetch_array(MYSQLI_ASSOC);
...
return $response['lastseen'] > strtotime("-1 minute");
}
...
$content = $this->template->fetch($this->theme_dir."/messenger/messenger-messages-tick.html");
...
public function getShortcutsSessionUserInfo(){
 ...
 private function cltype($type,$id, $b = false){
	...
	$call_back_audio = '<A href="javascript:void(0);" ontouchstart="setTimeout(function(){vy_calls.makeCall(event,\'audio\','.$id.');},100);" onclick="return vy_calls.makeCall(event,\'audio\','.$id.');">'.$str.'</a>';
	$call_back_video = '<A href="javascript:void(0);" ontouchstart="setTimeout(function(){vy_calls.makeCall(event,\'video\','.$id.');},100);" onclick="return vy_calls.makeCall(event,\'video\','.$id.');">'.$str.'</a>';
...
public function str_messenger($string, $min = false, $limit_chars = false, $only_txt = false){
	...
	$string =  preg_replace_callback("/\[missedcall\]((\s|.)+?)\[\/missedcall\]/i", function($k) use(&$min,&$_missedcall,&$ic_missedcall) { 
			...
			return $min ? $msg : '<div class="messenger_call_msg">'.sprintf($ic_missedcall,'').$msg.'<div class="messenger_call_msg_footer">'.$this->cltype($call_type,$recipient,$b).'</div></div>';
		}, $string);
	
	
	$string =  preg_replace_callback("/\[callended\]((\s|.)+?)\[\/callended\]/i", function($k) use(&$min,&$_called,&$ic_called) { 
			...
			return $min ? $msg : '<div class="messenger_call_msg">'.sprintf($ic_called,'').$msg.'<div class="messenger_call_msg_time">'.$call_duration.'</div><div class="messenger_call_msg_footer">'.$this->cltype($call_type,$recipient).'</div></div>';
		}, $string);
	...
	preg_replace_callback("/\[voice-clip\]((\s|.)+?)\[\/voice-clip\]/i", function($src)  {
		...
		return '<div class="sp-voice-clip-comment"><audio controls controlsList="nodownload" src="'.$this->media_url.$src.'"></audio></div>';
	...
	$string = $min ? preg_replace("/\[img\]((\s|.)+?)\[\/img\]/i", $only_txt ? "{$c} image" : "<div class=\"msg-media-txt\">".$svgI['contacts']['picture']."</div>",$string) : preg_replace("/\[img\]((\s|.)+?)\[\/img\]/i", "<div  data-src=\"".$this->settings['HOST']."/messenger.php?cmd=atch&id=\\1\" class=\"messenger-media-image\"><a class=\"messenger-image-hover\" target=\"_blank\" href=\"".$this->settings['HOST']."/messenger.php?cmd=atch&id=\\1\">  <img class=\"vy_ms__attch_img vy_ms_img_lzyload\"  src=\"".$this->settings['HOST']."/messenger.php?cmd=atch&low=1&id=\\1\" />  </a></div>", $string);//<div class=\"messenger-image-url\" style=\"background-image:url(".$this->settings['HOST']."/messenger.php?cmd=atch&id=\\1);\"></div>
	...
	$string = $min ? preg_replace("/\[video\]((\s|.)+?)\[\/video\]/i", $only_txt ? "{$cv} video" : "<div class=\"msg-media-txt\">".$svgI['contacts']['video']."</div>",$string) 
				...
				preg_replace_callback("/\[video\]((\s|.)+?)\[\/video\]/i", function($video_id) {
					...
					$id = $video_id[1];
					$video_file = "";
					$html = "<video class=\"lg-video-object lg-html5\" controls preload=\"metadata\">Video Not found</video>";
					if(is_numeric($id) && $id > 0){
						...
						$html = '<video class="lg-video-object lg-html5" controls preload="metadata">
	...
	$string = preg_replace("/\[gif\]((\s|.)+?)\[\/gif\]/i", "<div class=\"msg_gif\"><A href=\"$1\" target=\"_blank\"><img class=\"msg0xf3gif\" src=\"$1\" /><div class=\"gif_bottom_d\">GIF</div></a></div>", $string);
	}
	...
	return '<div class="js_mess_map_loc"><div class="messenger_map_location"><iframe src="https://maps.google.com/maps?q='.$lat.','.$lng.'&z=10&output=embed" width="360" height="270" frameborder="0" style="border:0"></iframe><div rel="li-gliph-color-background" class="messenger_map_info">'.$this->lang['My_location'].'</div></div></div>'; 
...
public function initiateCall($return = 1){
	...
	$q = $this->db->query("select `unique_id` as id,`status` from ".tbl_peer_id." where `userid`='{$this->recipient}' limit 1");
	$r = $q->fetch_array(MYSQLI_ASSOC);
	
	$response = array("peer_id" => (isset($r['id']) ? $r['id'] : 0), "blacklist" => $this->checkForBlacklist($this->recipient), "status" => (isset($r['status']) ? $r['status'] : 'not-available') );
		...
		foreach($q as $r):
							$message = array(
								...
								//if($r['page_id'] > 0){
									...
									//$count_by_users[$r['from_id'].'_'.$r['page_id']] = $r['msg_count'];
							//	}
...
public function str_messenger_wo_sync_media($media,$media_name,$min = false){
	...
	if(in_array(str_replace(".","",strtoupper($ext)),$pic)){
		...
		$media = "<video class=\"lg-video-object lg-html5\" controls preload=\"metadata\">
		...
		$media = "<div class=\"sp-voice-clip-comment\"><audio controls controlsList=\"nodownload\" src=\"".$this->settings['HOST']."/".$media."\"></audio></div>";
		$min_str = "voice clip";
...
public static function out_getMyMutedContacts(){
	$messenger = new MESSENGER; 
	return $messenger->getMyMutedContacts();
}
	...
	if(trim($url) && filter_var($url, FILTER_VALIDATE_URL)){
		...
		$urlExt = pathinfo($url, PATHINFO_EXTENSION);
		 if (in_array(strtolower($urlExt), $imgExts)) {
			...
			$arr_data = [ "embera" => "no", "send_now" => "yes", "id" => $id, "upload_url" => '/messenger.php?cmd=atch&id='.$id.'&type=image', "phext"=>$urlExt, "phFilename"=>$newImageName, "description" => $url, "type" => "image"];
		...
		$urlExt = pathinfo($url, PATHINFO_EXTENSION);
		 if (in_array(strtoupper($urlExt), $vExts)) {
			...
			$arr_data = [ "embera" => "no", "send_now" => "yes", "id" => $id, "upload_url" => '/messenger.php?cmd=atch&id='.$id.'&type=video', "phext"=>$urlExt, "phFilename"=>$newVideoName, "description" => $url, "type" => "video"];
			if($return) return $this->je($arr_data); else echo $this->je($arr_data);
			}
...
$content = $this->template->fetch($this->theme_dir."/messenger/calls/call.html");
...
public function callErrorMsg(){
		...
		$content = $this->template->fetch($this->theme_dir."/messenger/calls/error.html");
...
public function AnswerPopup(){
	...
	$this->template->assign(['this' => $this, 'user' => $metadata, 'media_type' => $metadata['call_type'] ]);
	$content = $this->template->fetch($this->theme_dir."/messenger/calls/answer.html");
