...
//console.log(ctx.globalconfig)
  ...
  ctx.globalconfig["nodejs_key_path"] =  configFile.nodejs_key
  ctx.globalconfig["nodejs_cert_path"] =    configFile.nodejs_cert
...
async function main() {
  ...
  if (ctx.globalconfig["redis"] === "Y") {
    // const redisAdapter = require('socket.io-redis');
    // io.adapter(redisAdapter({ host: 'localhost', port: ctx.globalconfig["redis_port"] }));
    const pubClient = createClient({ url: `redis://localhost:${ctx.globalconfig["redis_port"]}`});
