...
class phpVimeo
{
    ...
    private function _request($method, $call_params = array(), $request_method = 'GET', $url = self::API_REST_URL, $cache = true, $use_auth_header = true)
    {
        ...
        if ($use_auth_header) {
            $curl_opts[CURLOPT_HTTPHEADER] = array(
                $this->_generateAuthHeader($oauth_params)
            );
        }
        // Call the API
    ...
    {
        $access_token = $this->_request(null, array(
    ...
    {
        $request_token = $this->_request(null, array(
    ...
    public function upload($file_path, $use_multiple_chunks = false, $chunk_temp_dir = '.', $size = 2097152, $replace_id = null)
    {
        ...
        // Verify
        $verify = $this->call('vimeo.videos.upload.verifyChunks', array(
            'ticket_id' => $ticket
        ));
