(function(a) {
  ...
  function rate_manipulation(){
	    ...
	    rate_this_artist(f,jQuery(this).closest('.artist_rating').find('input:first').data('artist-numb'),g);
        });
...
if(jQuery('#rate_artist').length != 0 && jQuery('[data-artist-numb]').length != 0){
        jQuery.ajax({
            url: settings.musicpth + settings.manageMyMusic,
            type: 'POST',
            data: {
                action: 'check_rate',
                track: jQuery('#rate_artist').data('artist-numb')
            },
            cache: false,
