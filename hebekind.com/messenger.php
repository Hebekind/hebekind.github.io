...
try {
	...
	$ot = isset($_POST['ot']) && $_POST['ot'] == 'yes' ? 1 : 0;
	$json_format = isset($_POST['json_format']) && $_POST['json_format'] == 'yes' ? 1 : 0;
	$direct_media_url = isset($_POST['dmurl']) && $_POST['dmurl'] == 'yes' ? 1 : 0;
	$direct_url = '';
	...
	if(empty($string)) {
		...
		$res = $messenger->str_messenger($string,$min,0,$ot);
		if($direct_media_url)
			$direct_url = $messenger->extract_urlbbcode($string);
		
	}
	
	if($json_format)
		echo json_encode(["str" => $res, "direct_url" => $direct_url]);
	else
		echo $res;
	...
	break;
	case 'get-allmy-contacts-grouped':
	$messenger->getAllMyContactsGrouped();
	break;
	...
	$messenger->inviteToGroup();
	break;
	case 'accept-group-invitation':
	$messenger->acceptGroupInvitation();
	break;
	case 'ignore-group-invitation':
	$messenger->ignoreGroupInvitation();
	break;
	case 'get-cache-message':
	echo $messenger->getCacheMessage();
	break;
	case 'reaction':
	$messenger->reaction();
	break;
	case 'getReactedPeople':
	$messenger->getReactedPeople();
	break;
	case 'group-chat-search-participants':
	echo $messenger->groupChatFindParticipants();
	break;
	case 'create-group-chat':
	echo $messenger->createGroupChat();
	break;
	case 'edit-group-chat':
	echo $messenger->editGroupChat();
	break;
	case 'online-users-details':
	$messenger->onlineUsersDetails();
	break;
	case 'get-msg-details': 
	echo $messenger->getMessageDetails();
	break;
	case 'get-reply-details':
	echo $messenger->je($messenger->getReplyMessageText($id));
	break;
	case 'update-user-online-status':
	$messenger->updateUserOnlineStatus();
	break;
	case 'getuserLastActivity':
	echo $messenger->getuserLastActivity();
	break;
	case 'check_count_unread_msg':
	echo $messenger->checkCountDinCindInCind();
	break;
